...
Replace old with new in .txt files:

```bash
perl -ipe 's/old/new/g;' *.txt
```

destructive, though so you have to either do it right the first time or say
...
The `e` option says that what comes next is a one line program.
...
After the program comes the files I want to operate on, all files ending in `.txt` by using the wildcard character `*` that matches anything in a filename. Unfortunately, there are two contexts for using `*`. In a filename on the command line it stands for *anything* but in a regular expression it stands for *zero or more occurrences of the element that preceded it*. This causes a lot of confusion for users of regular expressions!
